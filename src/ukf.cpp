...
UKF::UKF() {
	...
	*/
	...
	time_us_ = 0;
	num_radar_updates = 0;
	radar_updates_with_good_nis = 0;
	num_lidar_updates = 0;
	lidar_updates_with_good_nis = 0;
}
	...
	****************************************************************************/
	...
	{	
		...
		if(use_radar_ == true)
		{
			...
			float nis_pct_radar = (float)radar_updates_with_good_nis/(float)num_radar_updates;
			cout << "nis_pct_radar = " << nis_pct_radar << endl;
		}
	} 
	else 
	{
		...
		if(use_laser_ == true)
		{
			...
			float nis_pct_lidar = (float)lidar_updates_with_good_nis/(float)num_lidar_updates;
			cout << "nis_pct_lidar = " << nis_pct_lidar << endl;
		}
	}
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
	...
	*/
	...
	UpdateStateLidar(Zsig,z_pred,S,meas_package.raw_measurements_,2);	
	float nis = CalcNISLidar(z_pred,S,meas_package.raw_measurements_,2);
	cout << "nis = " << nis << endl;
	
	if(nis <= 5.991)
	{
		lidar_updates_with_good_nis++;
	}
}
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
	...
	*/
	...
	float nis = CalcNIS(z_pred,S,meas_package.raw_measurements_,3);
	cout << "nis = " << nis << endl;
	
	if(nis <= 7.815)
	{
		radar_updates_with_good_nis++;
	}
}
...
MatrixXd Zsig,
 VectorXd z_pred,
 MatrixXd S, VectorXd raw_measurements_,
 int n_z) 
{
...
float UKF::CalcNIS(VectorXd z_pred, MatrixXd S, VectorXd raw_measurements_,int n_z)
{
	//create example vector for incoming radar measurement
	VectorXd z = VectorXd(n_z);
	z = raw_measurements_;
...
float UKF::CalcNISLidar(VectorXd z_pred, MatrixXd S, VectorXd raw_measurements_,int n_z)
{
	//create example vector for incoming radar measurement
	VectorXd z = VectorXd(n_z);
	z = raw_measurements_;
